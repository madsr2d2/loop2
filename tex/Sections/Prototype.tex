...
To validate the feasibility and user experience of the Energy-Aware Assistance System (EAAS), we developed a fully functional web-based prototype (\verb|`eaas_ui_prototype.html`|) that simulates the tractor's operating environment. This high-fidelity prototype demonstrates how the system processes sensor data, detects tasks, and provides real-time feedback to the operator.
...
\begin{figure}[ht]
	...
	\caption{Conceptual UI Prototype of the Energy-Aware Assistance System (EAAS). The display shows key operational data, including the current task, a real-time efficiency "Traffic Light" indicator, actionable suggestions for energy saving, and detailed telemetry.}
	\label{fig:prototype}
\end{figure}
...
\subsubsection*{Actionable Suggestions}
When the system detects inefficiency (Yellow/Red state), it generates a "Suggestion Card." Crucially, this card quantifies the benefit (e.g., "Energy: Will add ~12 min runtime") and offers a single "Apply Now" button. This reduces the friction of making an adjustment—the operator does not need to guess the correct setting, only to approve the system's recommendation.

\subsubsection*{Positive Reinforcement}
Drawing on behavioral psychology, the UI emphasizes rewards over penalties. A "Green Zone Streak" counter tracks how long the operator has maintained optimal efficiency, and an "Energy Saved" metric accumulates throughout the shift. This gamification element encourages operators to maintain their streak, turning energy saving into a personal achievement.
