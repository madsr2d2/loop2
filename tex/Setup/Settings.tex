...
\colorlet{frontbackcolor}{orange} % Set the background colour of the front- and back page. Choose the colour so it matches the main colour of front page picture
...
\setcounter{tocdepth}{3}    % Depth of table of content: sub sections will not be included in table of contents
...
\fancypagestyle{main}{% All normal pages
    ...
    \fancyfoot[RE,LO]{\footnotesize \thesistitle: \thesissubtitle} % - \rightmark
...
\begin{center}
    ...
    % uncomment this skip when using picture of signature
    %\vspace{1.5cm} % Spacing between name and signature line 

    % uncomment the following to include picture of signature
    \includegraphics[width=5cm,keepaspectratio]{Pictures/mySig.png}
...
% One author box: date defaults to \today
% Usage: \authorentry[<Date>]{<Name>}{<Student No.>}
\NewDocumentCommand{\authorentry}{ O{\today} m m }{%
...
% Convenience wrapper to lay out authors on one line (auto spacing)
% Usage: \authorsline{ \authorentry{Alice}{s123456} \authorentry{Bob}{s654321} }
